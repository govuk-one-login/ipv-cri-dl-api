...
mkdir kmsEncryption
privateKey=$(aws-vault exec $ACCOUNT_ID -- aws acm export-certificate --certificate-arn $ENCRYPTION_CERT_ACM --passphrase fileb://passPhrase.txt | jq -r '"\(.PrivateKey)"')
...
aws-vault exec $ACCOUNT_ID -- aws kms import-key-material --key-id $kmsEncryptionKeyId \
