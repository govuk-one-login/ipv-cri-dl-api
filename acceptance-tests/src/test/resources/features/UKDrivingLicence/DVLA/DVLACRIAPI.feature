...
Feature: DrivingLicence CRI API
  ...
  Scenario Outline: DVLA Driving Licence - Auth Source Happy path
    ...
    When Driving Licence user sends a POST request to Driving Licence endpoint using updated jsonRequest returned from the personInfo Table DVLAValidKennethJsonPayload
    ...
    Examples:
      | context       | personalNumber   | expiryDate | issueDate  | issueNumber | issuedBy | fullAddress                 |
      | check_details | DOE99751010AL9OD | 2022-02-02 | 2012-02-02 | 13          | DVLA     | 8 HADLEY ROAD BATH TB2 5AA  |
